...
const availableLanguages: string[] = ['en', 'de', 'zh-CN'];
...
/**
 * OPTIMIZATION: Lazy-load GUI only when needed
 * By default, the application starts with only the tray icon
 * The GUI is loaded on-demand when the user clicks the tray icon
 * This significantly reduces memory usage at startup
 */
async function initMain(): Promise<void> {
    // Only load GUI if --tray flag is NOT present (backward compatibility)
    // Default behavior: start with tray only, load GUI on demand
    if (!trayOnlyOption) {
        console.log('initMain: TUXEDO Control Center started (tray mode)');
        console.log('initMain: Click tray icon to open the GUI');
        // GUI will be loaded when user clicks the tray icon via activateTccGui()
    } else {
        console.log('initMain: TUXEDO Control Center started (tray-only mode)');
    }
