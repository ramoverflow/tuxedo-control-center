...
export class KeyboardBacklightComponent implements OnInit {
    ...
    public chosenMode: KeyboardBacklightColorModes = KeyboardBacklightColorModes.static;
    public chosenSpeed: number = 128;
    public KeyboardBacklightColorModes = KeyboardBacklightColorModes; // Expose enum to template
    ...
    private setKeyboardBacklightStates(
        ...
        if (keyboardBacklightStates?.[0]) {
            const { brightness, mode, speed } = keyboardBacklightStates[0];
            ...
            if (hasChosenColor && hasNoPickerInUsage) {
                ...
                // ENHANCED: Load animation mode and speed
                this.chosenMode = mode ?? KeyboardBacklightColorModes.static;
                this.chosenSpeed = speed ?? 128;
            } else {
    ...
    private fillKeyboardBacklightStatesFromValues(
        brightness: number,
        colorHex: string[] = [],
        mode: KeyboardBacklightColorModes = KeyboardBacklightColorModes.static,
        speed: number = 128,
    ...
    public onBrightnessSliderInput(brightness: number): void {
        ...
        this.tccdbus.setKeyboardBacklightStates(
            this.fillKeyboardBacklightStatesFromValues(brightness, colorHex, this.chosenMode, this.chosenSpeed),
        );
    }
    ...
    // ENHANCED: Animation mode change handler
    public onModeChanged(newMode: KeyboardBacklightColorModes): void {
        this.chosenMode = newMode;
        const colorHex = this.chosenColorHex?.length ? this.chosenColorHex : undefined;
        this.tccdbus.setKeyboardBacklightStates(
            this.fillKeyboardBacklightStatesFromValues(
                this.chosenBrightness,
                colorHex,
                newMode,
                this.chosenSpeed,
    ...
    // ENHANCED: Animation speed change handler
    ...
    public getModeLabel(mode: KeyboardBacklightColorModes): string {
        switch (mode) {
