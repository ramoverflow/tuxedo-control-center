...
export class KeyboardBacklightListener {
    ...
    private updateKeyboardBacklightCapabilities(): void {
        ...
        // ENHANCED: Detect supported animation modes
        this.keyboardBacklightCapabilities.modes = this.detectSupportedModes();
    ...
    /**
     * ENHANCEMENT: Detect which animation modes are supported by the driver
     */
    private detectSupportedModes(): KeyboardBacklightColorModes[] {
        const supportedModes: KeyboardBacklightColorModes[] = [KeyboardBacklightColorModes.static];
        
        // Check if RGB keyboard is available (supports animations)
        if (this.ledsRGBZones?.length > 0 && fileOK(`${this.ledsRGBZones[0]}/max_brightness`)) {
            // Check for effect control file
            if (fileOK(`${this.ledsRGBZones[0]}/effect`)) {
                console.log('KeyboardBacklightListener: Detected animation support');
                supportedModes.push(KeyboardBacklightColorModes.breathing);
                supportedModes.push(KeyboardBacklightColorModes.wave);
                // Add more modes based on driver capabilities
                if (fileOK(`${this.ledsRGBZones[0]}/reactive`)) {
                    supportedModes.push(KeyboardBacklightColorModes.reactive);
                }
                if (fileOK(`${this.ledsRGBZones[0]}/spectrum`)) {
                    supportedModes.push(KeyboardBacklightColorModes.spectrum);
                }
            }
        }
    ...
    protected async setKeyboardBacklightStates(
        ...
        if (updateSysFS) {
            ...
            for (let i: number = 0; i < this.ledsRGBZones?.length; ++i) {
                if (this.ledsRGBZones[i]) {
                    const state = keyboardBacklightStatesNew?.[i];
                    if (!state) continue;

                    // ENHANCED: Apply effect based on animation mode
                    switch (state.mode) {
                        case KeyboardBacklightColorModes.static:
                            await this.setStaticEffect(this.ledsRGBZones[i], state);
                            break;
                        case KeyboardBacklightColorModes.breathing:
                            await this.setBreathingEffect(this.ledsRGBZones[i], state);
                            break;
                        case KeyboardBacklightColorModes.wave:
                            await this.setWaveEffect(this.ledsRGBZones[i], state);
                            break;
                        case KeyboardBacklightColorModes.reactive:
                            await this.setReactiveEffect(this.ledsRGBZones[i], state);
                            break;
                        case KeyboardBacklightColorModes.spectrum:
                            await this.setSpectrumEffect(this.ledsRGBZones[i], state);
                            break;
                        default:
                            await this.setStaticEffect(this.ledsRGBZones[i], state);
                    }
    ...
    /**
     * ENHANCEMENT: Static effect - solid color
     */
    private async setStaticEffect(
        ledPath: string,
        state: KeyboardBacklightStateInterface,
    ): Promise<void> {
        if (await fileOKAsync(`${ledPath}/multi_intensity`)) {
            const red = state.red.toString();
            const green = state.green.toString();
            const blue = state.blue.toString();
            await fs.promises.appendFile(
                `${ledPath}/multi_intensity`,
                `${red} ${green} ${blue}`,
            );
        }
    }
    ...
    /**
     * ENHANCEMENT: Breathing effect - color fades in and out
     */
    private async setBreathingEffect(
        ledPath: string,
        state: KeyboardBacklightStateInterface,
    ): Promise<void> {
        try {
            // Set effect type to breathing (usually 1)
            if (await fileOKAsync(`${ledPath}/effect`)) {
                await fs.promises.appendFile(`${ledPath}/effect`, '1');
            }
            ...
            // Set animation speed
            if (await fileOKAsync(`${ledPath}/speed`)) {
                const speed = (state.speed ?? 128).toString();
                await fs.promises.appendFile(`${ledPath}/speed`, speed);
            }
        } catch (err: unknown) {
            console.error(`KeyboardBacklightListener: setBreathingEffect failed => ${err}`);
        }
    }

    /**
     * ENHANCEMENT: Wave effect - color travels across keyboard
     */
    private async setWaveEffect(
        ledPath: string,
        state: KeyboardBacklightStateInterface,
    ): Promise<void> {
        try {
            // Set effect type to wave (usually 2)
            if (await fileOKAsync(`${ledPath}/effect`)) {
                await fs.promises.appendFile(`${ledPath}/effect`, '2');
            }
            ...
            console.error(`KeyboardBacklightListener: setWaveEffect failed => ${err}`);
        }
    }

    /**
     * ENHANCEMENT: Reactive effect - keys light up when pressed
     */
    private async setReactiveEffect(
        ledPath: string,
        state: KeyboardBacklightStateInterface,
    ): Promise<void> {
        try {
            if (await fileOKAsync(`${ledPath}/effect`)) {
                await fs.promises.appendFile(`${ledPath}/effect`, '3');
            }
            ...
            console.error(`KeyboardBacklightListener: setReactiveEffect failed => ${err}`);
        }
    }

    /**
     * ENHANCEMENT: Spectrum effect - rainbow color cycling
     */
    private async setSpectrumEffect(
        ledPath: string,
        state: KeyboardBacklightStateInterface,
    ): Promise<void> {
        try {
            if (await fileOKAsync(`${ledPath}/effect`)) {
                await fs.promises.appendFile(`${ledPath}/effect`, '4');
            }
            ...
            console.error(`KeyboardBacklightListener: setSpectrumEffect failed => ${err}`);
        }
    }
}
